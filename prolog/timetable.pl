...
scheduled_hours(C, Count) :-
    findall(1, schedule(C, _, _), L),
    length(L, Count).

verify_schedule :-
    nl, write('=========== VERIFICATION ==========='), nl,
    forall(
        course(C, Name, _),
        ( required_hours(C, Req),
          scheduled_hours(C, Got),
          ( Got >= Req
          -> format('~w (~w): OK ~w/~w~n', [C, Name, Got, Req])
          ;  format('~w (~w): INCOMPLETE ~w/~w~n', [C, Name, Got, Req])
          )
        )
    ),
    write('====================================='), nl.
